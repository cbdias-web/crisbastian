...
export default function Espelhamentos() {
  ...
  // Período para relatórios (oculto do usuário mas necessário para as funções de PDF)
  const hoje = new Date();
  const dataInicio = `${hoje.getFullYear()}-${String(hoje.getMonth() + 1).padStart(2, "0")}-01`;
  const lastDay = new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0).getDate();
  const dataFim = `${hoje.getFullYear()}-${String(hoje.getMonth() + 1).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}`;
  ...
  const { data: indicadores = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await base44.entities.Espelhamento.list();
      return result || [];
    },
    refetchOnMount: true,
    staleTime: 0,
  ...
  const { data: vendas = [] } = useQuery({
    queryKey: ['vendas-esp'],
    queryFn: async () => {
      const result = await base44.entities.Venda.list('-data');
      return result || [];
    },
    staleTime: 30000,
  });
  ...
  const gerarRelatorioGeral = async () => {
    const base = selectedForRelatorio.length > 0
      ? visibleIndicadores.filter(e => selectedForRelatorio.includes(e.id))
      : visibleIndicadores;
    if (base.length === 0) {
    ...
    try {
      const response = await base44.functions.invoke('gerarRelatorioComissoesPDF', {
        indicadores_ids: base.map(e => e.id),
        indicadores_nomes: base.map(e => e.nome),
        vendedores_ids: [],
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `relatorio-indicadores.pdf`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success('Relatório gerado!');
    } catch (error) {
  ...
  const enviarRelatorioPorEmail = async (indicador) => {
    ...
    if (!confirm(`Enviar relatório para ${indicador.email}?`)) return;
  ...
  const toggleAllForRelatorio = () => {
    setSelectedForRelatorio(prev =>
      prev.length === visibleIndicadores.length ? [] : visibleIndicadores.map(i => i.id)
    );
  };
  ...
  const abrirModalEnvio = () => {
    const comEmail = visibleIndicadores.filter(i => i.email);
    if (comEmail.length === 0) {
      toast.error('Nenhum indicador com e-mail cadastrado');
      return;
    }
    const historicoKey = `envios_relatorios_indicadores`;
  ...
  const enviarRelatoriosEmMassa = async () => {
    const selecionados = visibleIndicadores.filter(i => selectedForEmail.includes(i.id) && i.email);
    setSendingBulk(true);
    let sucessos = 0;
    let erros = 0;
    const novosEnvios = [];
    for (const i of selecionados) {
      try {
        ...
        sucessos++;
        novosEnvios.push({ vendedor_id: i.id, vendedor_nome: i.nome, vendedor_email: i.email, data_envio: new Date().toISOString() });
      } catch {
        erros++;
      }
    }
  ...
  // Filtra por status e busca — SEM filtro de período
  const visibleIndicadores = indicadores.filter(i => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto space-y-5">
          ...
          {isAdmin && (
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                ...
                {selectedForRelatorio.length > 0 ? `Relatório (${selectedForRelatorio.length})` : 'Relatório Geral'}
              </button>
              <button
        ...
        {/* Filtros — apenas busca e status, SEM período */}
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 py-16 text-center text-gray-400">
            ...
            <p className="text-xs text-gray-300 mt-1">
              {indicadores.length === 0 ? 'Nenhum indicador cadastrado na base de dados.' : 'Tente ajustar os filtros.'}
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="flex items-center gap-3 px-1">
              <input type="checkbox"
                checked={selectedForRelatorio.length === visibleIndicadores.length && visibleIndicadores.length > 0}
                onChange={toggleAllForRelatorio}
                className="w-4 h-4 accent-[#1a3150] cursor-pointer"
              />
              <span className="text-xs text-gray-500">
                {selectedForRelatorio.length === 0
                  ? `Selecionar todos para relatório (${visibleIndicadores.length})`
                  : `${selectedForRelatorio.length} selecionado(s)`}
              </span>
              {selectedForRelatorio.length > 0 && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {visibleIndicadores.map(i => {
                const vendasDoIndicador = vendas.filter(v =>
                  ...
                  <div key={i.id} className={`bg-white rounded-2xl shadow-sm border p-5 hover:shadow-md transition ${selectedForRelatorio.includes(i.id) ? 'border-[#1a3150]/40 ring-1 ring-[#1a3150]/20' : 'border-gray-100'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      <div className="text-center p-2 bg-gray-50 rounded-xl">
                        <p className="text-lg font-bold text-gray-900">{vendasDoIndicador.length}</p>
                        <p className="text-[10px] text-gray-400">Vendas totais</p>
                      </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Selecione os indicadores que receberão o relatório</p>
                </div>
              ...
              <div className="p-6 overflow-y-auto max-h-[calc(80vh-200px)]">
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    {visibleIndicadores.filter(i => i.email).map(i => {
                      ...
                      return (
                        <div key={i.id} className={`flex items-center gap-3 p-3 rounded-lg ${foiEnviado ? 'bg-green-50 border border-green-200' : 'bg-gray-50'}`}>
                          <input type="checkbox" checked={selectedForEmail.includes(i.id)} onChange={() => toggleSelectForEmail(i.id)}
              ...
              <div className="px-6 py-4 border-t border-gray-100 flex justify-between items-center">
                <p className="text-sm text-gray-600">{selectedForEmail.length} selecionado(s)</p>
                <div className="flex gap-2">
                  ...
                  <button onClick={enviarRelatoriosEmMassa} disabled={sendingBulk || selectedForEmail.length === 0}
                    ...
                    {sendingBulk ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Enviando...</> : <><Send className="w-4 h-4" />Enviar</>}
                  </button>
                </div>
              </div>
            </div>
