...
import { MessageSquare, Zap, Users, RefreshCw, CheckCircle2, Filter, Search, Sparkles, Phone, Clock, Copy } from 'lucide-react';
...
const STATUS_COLORS = {
  ...
  qualificado: { bg: 'rgba(139,92,246,0.2)', color: '#a78bfa', label: 'Qualificado IA' },
...
export default function CentralLeads() {
  ...
  const { data: conversas = [], isLoading, refetch } = useQuery({
    queryKey: ['conversas-whatsapp', user?.id],
    queryFn: async () => {
      if (!user) return [];
      if (isAdmin) {
        return base44.entities.ConversaWhatsapp.list('-ultima_mensagem_em', 100);
      }
      // Vendedor vê só as suas
      const vends = await base44.entities.Vendedor.filter({ email: user.email });
      if (!vends.length) return [];
      return base44.entities.ConversaWhatsapp.filter({ vendedor_id: vends[0].id }, '-ultima_mensagem_em');
    },
    enabled: !!user,
  ...
  useEffect(() => {
    const unsub = base44.entities.ConversaWhatsapp.subscribe(() => {
      refetch();
    });
    return unsub;
  }, []);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm" style={{ color: AURORA.textMuted }}>Leads chegando via campanha externa · WhatsApp integrado</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={copiarWebhook}
              ...
              <Copy className="w-3.5 h-3.5" /> URL do Webhook
            </button>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            { label: 'Qualificados pela IA', value: totalQualificados, icon: Sparkles, color: '#a78bfa' },
        ...
        <div className="flex gap-4">
          ...
          <div className="flex flex-col gap-3" style={{ width: conversaSelecionada ? '380px' : '100%', flexShrink: 0 }}>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12 rounded-2xl" style={{ background: AURORA.surface, border: `1px solid ${AURORA.border}` }}>
                ...
                <p className="text-sm mt-1" style={{ color: AURORA.textMuted }}>Os leads virão automaticamente via webhook do Make</p>
              </div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: isSelected ? `1px solid ${AURORA.accent}` : `1px solid ${AURORA.border}`,
                      }}
                      onMouseEnter={e => { if (!isSelected) e.currentTarget.style.borderColor = 'rgba(0,212,170,0.3)'; }}
                      onMouseLeave={e => { if (!isSelected) e.currentTarget.style.borderColor = AURORA.border; }}
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              ...
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: stCfg.bg, color: stCfg.color }}>
                                {stCfg.label}
                              </span>
                            </div>
                          </div>
                          <p className="text-xs truncate mt-0.5" style={{ color: AURORA.textMuted }}>
                          ...
                          <div className="flex items-center justify-between mt-1.5">
                            ...
                            {conv.status === 'ativa' && (
                              <button
                                onClick={(e) => marcarQualificado(conv, e)}
          ...
          {/* Chat */}
          {conversaSelecionada && (
            <div className="flex-1 rounded-2xl overflow-hidden" style={{ border: `1px solid ${AURORA.border}`, height: 'calc(100vh - 220px)' }}>
              <ChatConversa
        ...
        {/* Guia de integração */}
        <div className="mt-8 rounded-2xl p-5" style={{ background: AURORA.surface, border: '1px solid rgba(0,212,170,0.2)' }}>
          <h3 className="font-bold text-sm mb-3 flex items-center gap-2" style={{ color: AURORA.accent }}>
            <Zap className="w-4 h-4" /> Guia de Integração — Make + Z-API
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs" style={{ color: AURORA.textMuted }}>
            <div>
              <p className="font-bold mb-2" style={{ color: AURORA.text }}>📥 Webhook: Receber novo lead</p>
              <div className="rounded-xl p-3 font-mono text-[11px]" style={{ background: '#0d1117', border: '1px solid rgba(0,212,170,0.1)' }}>
                <p style={{ color: '#f87171' }}>POST</p>
                <p style={{ color: '#00D4AA' }}>/api/functions/receberLeadExterno</p>
                <p className="mt-2" style={{ color: '#fbbf24' }}>Header: x-webhook-token: SEU_TOKEN</p>
                <p className="mt-2" style={{ color: AURORA.textMuted }}>{'{'}</p>
                <p className="ml-2" style={{ color: AURORA.textMuted }}>"nome": "João Silva",</p>
                <p className="ml-2" style={{ color: AURORA.textMuted }}>"telefone": "5511999998888",</p>
                <p className="ml-2" style={{ color: AURORA.textMuted }}>"produto_interesse": "Conta Global",</p>
                <p className="ml-2" style={{ color: AURORA.textMuted }}>"observacao_ia": "Cliente interessado em...",</p>
                <p className="ml-2" style={{ color: AURORA.textMuted }}>"origem": "campanha_junho"</p>
                <p style={{ color: AURORA.textMuted }}>{'}'}</p>
              </div>
            </div>
            <div>
              <p className="font-bold mb-2" style={{ color: AURORA.text }}>💬 Webhook: Receber mensagem do WhatsApp</p>
              <div className="rounded-xl p-3 font-mono text-[11px]" style={{ background: '#0d1117', border: '1px solid rgba(0,212,170,0.1)' }}>
                <p style={{ color: '#f87171' }}>POST</p>
                <p style={{ color: '#00D4AA' }}>/api/functions/receberMensagemWhatsapp</p>
                <p className="mt-2" style={{ color: '#fbbf24' }}>Header: x-webhook-token: SEU_TOKEN</p>
                <p className="mt-2" style={{ color: AURORA.textMuted }}>{'{'}</p>
                <p className="ml-2" style={{ color: AURORA.textMuted }}>"telefone": "5511999998888",</p>
                <p className="ml-2" style={{ color: AURORA.textMuted }}>"texto": "Olá, tenho interesse!"</p>
                <p style={{ color: AURORA.textMuted }}>{'}'}</p>
              </div>
            </div>
          </div>
