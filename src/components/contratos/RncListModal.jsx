...
export default function RncListModal({ onClose, onOpenRnc }) {
  ...
  const rncsFiltrados = rncs.filter(r => {
    ...
    if (busca.trim()) {
      ...
      return (r.nome || '').toLowerCase().includes(q) || (r.cpf_cnpj || '').toLowerCase().includes(q);
    }
    return true;
  });

  const submetidos = rncs.filter(r => r.link_preenchido_em).length;

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }}>
      <div className="w-full max-w-4xl max-h-[88vh] rounded-2xl overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold" style={{ color: AURORA.text }}>RNCs — Canal Bancário</h2>
              <p className="text-xs" style={{ color: AURORA.textMuted }}>
                {rncs.length} formulário(s) · {submetidos} preenchido(s) pelo cliente
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <FileCheck2 className="w-10 h-10 mx-auto mb-3" style={{ color: AURORA.textMuted, opacity: 0.3 }} />
              <p className="text-sm" style={{ color: AURORA.textMuted }}>Nenhuma RNC encontrada</p>
            </div>
          ) : (
            <div className="space-y-2">
              {rncsFiltrados.map(r => {
                const st = STATUS_CFG[r.status] || STATUS_CFG.rascunho;
                const isPJ = r.tipo_canal === 'PJ';
                  ...
                  <div key={r.id} className="rounded-xl p-4 flex items-center gap-3 transition hover:shadow-lg"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-0.5">
                        ...
                        <p className="text-sm font-semibold truncate" style={{ color: AURORA.text }}>{r.nome || '—'}</p>
                      ...
                      <p className="text-xs" style={{ color: AURORA.textMuted }}>
                        {r.cpf_cnpj || '—'} · {isPJ ? 'PJ' : 'PF'} · {docsOk}/{docsTotal} docs
