...
import { Loader2, Send, User, Building2, CheckCircle2, Zap, ArrowLeft } from 'lucide-react';
...
export default function FormularioIndicacao({ parceiro, onVoltar }) {
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (r?.success) {
        ...
        queryClient.invalidateQueries({ queryKey: ['indicador-leads-auth', parceiro?.id] });
    ...
    <div className="min-h-screen p-4 md:p-6" style={{ background: AURORA.bg, color: AURORA.text }}>
      <div className="max-w-3xl mx-auto">
        ...
        <button onClick={onVoltar}
          className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold mb-4 transition"
          style={{ background: AURORA.surface, color: AURORA.text, border: `1px solid ${AURORA.border}` }}>
          <ArrowLeft className="w-3.5 h-3.5" /> Voltar ao portal
        </button>

        {/* Cabeçalho */}
        <div className="flex flex-col items-center text-center mb-6">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-3" style={{ background: 'linear-gradient(135deg, #00D4AA, #0066cc)' }}>
            <Send className="w-7 h-7 text-white" />
          </div>
          <h1 className="text-xl font-bold" style={{ color: AURORA.text }}>Formulário de Indicação</h1>
          <p className="text-sm mt-0.5" style={{ color: AURORA.accent }}>Parceiro: {parceiro?.nome}</p>
          <p className="text-xs mt-1.5 max-w-md" style={{ color: AURORA.textMuted }}>
            Preencha os dados do potencial cliente. Você permanecerá vinculado a esta indicação.
          </p>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: AURORA.surface, border: `1px solid ${AURORA.border}` }}>
          {sucesso ? (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs leading-relaxed" style={{ color: AURORA.textMuted }}>
                      Sua indicação foi encaminhada para a nossa esteira comercial{sucesso.vendedor ? ` e já está com ${sucesso.vendedor}` : ''}. Acompanhe o andamento no portal.
                    </p>
                  </div>
                </div>
              </div>
              <button onClick={finalizar}
                className="w-full mt-5 px-4 py-3 rounded-xl text-sm font-bold transition"
            ...
            <div className="p-5 md:p-6 space-y-5">
              ...
              {tipo === 'PF' && (
                <div className="space-y-3 pt-3" style={{ borderTop: `1px solid ${AURORA.border}` }}>
              ...
              <button onClick={salvar} disabled={enviando}
                className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-bold transition disabled:opacity-40"
        ...
        <p className="text-center text-[11px] mt-5" style={{ color: AURORA.textMuted }}>
